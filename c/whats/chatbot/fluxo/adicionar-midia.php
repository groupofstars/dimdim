...
if($_FILES['imagem']){
	$directory = '../../images/';
 	$name = uniqid() . '.png';//pathinfo($_FILES['imagem']['name'], PATHINFO_EXTENSION);
  move_uploaded_file($_FILES['imagem']['tmp_name'], $directory . $name);
}


$insere = $pdo->query("INSERT INTO chatbot_responde (tipo, texto,midia, instancia, posicao, hash) VALUES ('midia', '$texto', '$name','$instancia', '$proxima_posicao', '$hash')");
