...
if($_FILES['pdf']){
	$directory = '../../images/';
 	$name = "doc-".uniqid() . '.' . pathinfo($_FILES['pdf']['name'], PATHINFO_EXTENSION);
  move_uploaded_file($_FILES['pdf']['tmp_name'], $directory . $name);
}


$insere = $pdo->query("INSERT INTO chatbot_responde (tipo, midia, instancia, posicao, hash) VALUES ('pdf', '$name', '$instancia', '$proxima_posicao', '$hash')");
